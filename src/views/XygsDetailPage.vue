...
export default {
	...
	mounted: function() {
		...
		let list = this.$store.state.listTab[`parentTab${pName}`].tabInfo[active].list
		// 当前行数据
		let currentData = list.filter((item) => {
			return item[key] == id
		})[0]
		...
		// console.log(obj);
		for (let ckey in currentData) {
			obj.map((item, index) => {
				if (ckey == item.field) {
					if (item.type == 'date') {
						this.newObj.push({
							name: item.name,
							value: new Date(currentData[ckey]).format('yyyy-MM-dd')
						})
					} else {
						this.newObj.push({
							name: item.name,
							value: currentData[ckey]
						})
					}
				}
			})
